...
EOF
        ...
        if [ $i -eq 0 ]; then
            ...
            # 配置kubelet
            green_echo "节点 $HOST 开始配置kubelet"
            ...
            if [ $i -eq 1 ]; then
                ...
                member add ${NAMES[1]} --peer-urls=https://${HOSTS[1]}:2380"
                ...
                --key=/etc/kubernetes/pki/etcd/peer.key \
                --endpoints=https://${HOSTS[0]}:2379 \
                member add ${NAMES[2]} --peer-urls=https://${HOSTS[2]}:2380"
    ...
    # 安装网络组件
    green_echo "集群开始安装calico"
    route_addr=`route -n|sed -n '3p'|awk '{print $2}'`
    cp ${SH_DIR}/../network/calico_tpl.yaml ${SH_DIR}/../network/calico.yaml
    sed -i "s/192.168.0.1/${route_addr}/g" ${SH_DIR}/../network/calico.yaml
    sed -i "s#10.0.0.0\/16#${podSubnet}#g" ${SH_DIR}/../network/calico.yaml
    kubectl apply -f ${SH_DIR}/../network/calico.yaml
    rm -rf ${SH_DIR}/../network/calico.yaml

    # 安装nginx-ingress-controller
    green_echo "集群开始安装nginx-ingress-controller"
    kubectl apply -f ${SH_DIR}/../network/ingress-nginx.yaml
